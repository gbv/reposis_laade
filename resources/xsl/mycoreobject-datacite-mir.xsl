...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="identifier">
    <identifier identifierType="DOI">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of
              select="concat('10.5072/dummy.',number(substring-after(substring-after(ancestor::mycoreobject/@ID,'_'),'_')))"/>
        </xsl:otherwise>
      </xsl:choose>
    </identifier>
  </xsl:template>
    ...
    <publisher>
      <xsl:choose>
        ...
        <xsl:when test="mods:name[$publisherRoles/@ID=mods:role/mods:roleTerm/text()]">
          <xsl:apply-templates select="mods:name[$publisherRoles/@ID=mods:role/mods:roleTerm/text()][1]"
                               mode="publisher"/>
        </xsl:when>
        <xsl:when test="mods:name[mods:role/mods:roleTerm/text()='his' and @valueURI]">
          <xsl:variable name="insti"
                        select="substring-after(mods:name[mods:role/mods:roleTerm/text()='his' and @valueURI]/@valueURI, '#')"/>
          <xsl:variable name="myURI" select="concat('classification:metadata:0:parents:mir_institutes:',$insti)"/>
          <xsl:variable name="cat"
                        select="document($myURI)//category[@ID=$insti]/ancestor-or-self::category[label[lang('x-place')]][1]"/>
  ...
  <xsl:template name="contributors">
    ...
    <contributors>
      ...
      <xsl:apply-templates select="mods:name[$contributorRoles/@ID=mods:role/mods:roleTerm/text()][1]"
                           mode="contributor"/>
    </contributors>
  </xsl:template>
  ...
  <xsl:template mode="contributor" match="mods:name">
    <contributor>
      <xsl:apply-templates select="mods:role/mods:roleTerm[@type='code' and @authority='marcrelator']"
                           mode="contributorType"/>
      <contributorName>
        <xsl:choose>
          <xsl:when test="mods:displayForm">
            <xsl:value-of select="mods:displayForm"/>
  ...
  <xsl:template name="dates">
    <xsl:if
        test="mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateOther|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateIssued|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateCreated|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateModified">
      <dates>
        <xsl:apply-templates
            select="mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateOther|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateIssued|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateCreated|mods:originInfo[not(@eventType) or @eventType='publication']/mods:dateModified"/>
      </dates>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="alternateIdentifiers">
    <alternateIdentifiers>
      ...
      <alternateIdentifier alternateIdentifierType="URI"><xsl:value-of select="mods:identifier[@type='uri']" /></alternateIdentifier>
  ...
  <xsl:template match="mods:identifier[not(@type='doi')]">
    <alternateIdentifier
        alternateIdentifierType="{translate(@type,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')}">
      <xsl:value-of select="."/>
    </alternateIdentifier>
  </xsl:template>
