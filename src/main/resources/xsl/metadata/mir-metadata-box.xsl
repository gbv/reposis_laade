  ...
  <xsl:template match="/">
    ...
    <div id="mir-metadata">
      ...
      <table class="mir-metadata">
        ...
        <xsl:for-each
            select="mcrxsl:distinctValues(mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:name[not(@ID) and
                      (@type='personal' or (@type='corporate' and not(@authorityURI = $institutesURI)))
                      and not(mods:role/mods:roleTerm='aut')]/mods:role/mods:roleTerm/text())">
          <xsl:variable name="role" select=".." />
          <!-- for every role -->
          <xsl:choose>
            <!-- check if 'aut' and 'edt' show 'edt', otherwise 'edt' is already shown in abstract-box -->
            <xsl:when test="$role='edt' and not(../../../mods:name/mods:role/mods:roleTerm='aut')">
              <!-- do nothing -->
            </xsl:when>
            <xsl:otherwise>
              <tr>
                <td valign="top" class="metaname">
                  <xsl:choose>
                    <xsl:when test="$role[@authority='marcrelator' and @type='code']">
                      <xsl:apply-templates select="$role[@authority='marcrelator' and @type='code']"
                                           mode="printModsClassInfo" />
                      <xsl:value-of select="':'" />
                    </xsl:when>
                    <xsl:when test="$role[@authority='marcrelator']">
                      <xsl:value-of
                          select="concat(i18n:translate(concat('component.mods.metaData.dictionary.',$role[@authority='marcrelator'])),':')" />
                    </xsl:when>
                  </xsl:choose>
                </td>
                <td class="metavalue">
                  <xsl:for-each select="key('name-by-role',$role)">
                    <xsl:if test="position()!=1">
                      <xsl:value-of select="'; '" />
                    </xsl:if>
